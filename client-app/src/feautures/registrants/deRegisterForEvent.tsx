...
export default observer(function DeRegisterForEvent() {
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const registrationData: Registration = await agent.Registrations.details(
        id!,
      );
      setRegistration(registrationData);
      const registrationEvent: RegistrationEvent =
        await agent.RegistrationEvents.details(
          registrationData.registrationEventId,
        );
          ...
          <Segment clearing color="teal">
            <ButtonGroup floated="right" size={isMobile ? 'tiny' : 'huge'}>
              <Button
                ...
                content="Go Back"
                onClick={() =>
                  navigate(`/registerforevent/${registrationEvent.id}`)
                }
              />
              <Button
                type="button"
        ...
        <SegmentGroup style={{ padding: '40px' }}>
          <Segment color="teal">
            <Header as="h2" icon textAlign="center">
              ...
              <HeaderSubheader>
                Your registration for {registrationEvent.title} has been
