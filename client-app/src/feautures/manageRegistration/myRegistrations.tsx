...
export default observer(function MyRegistrations() {
  ...
  const handleRemoveRegistrationEvent = (id: string) => {
    setRegistrationEvents(
      registrationEvents.filter((event) => event.id !== id),
    );
  };
    ...
    <>
      ...
      <Container fluid style={{ color: '#333', paddingTop: '20px' }}>
          ...
          <div style={{ textAlign: 'center', width: '100%' }}>
            ...
            >
              ...
              <Header.Subheader style={{ color: '#666' }}>
                ...
                <h3>
                  Edit your events or create new ones. Organize your upcoming
                  classes, symposiums, and registration-required activities.
                </h3>
              </Header.Subheader>
            </Header>
          </div>
        ...
        <CardGroup
          ...
          style={{ padding: isMobile ? '5px' : '40px' }}
        >
          {displayRegistrationEvents()
            .filter(
              (event) => new Date(event.endDate) >= new Date() || !showActive,
            )
            .map((event) => (
              <EventCard
                key={event.id}
                event={event}
                removeEvent={handleRemoveRegistrationEvent}
              />
            ))}
        </CardGroup>
      </Container>
