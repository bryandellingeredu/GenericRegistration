...
export default observer(function SendEmailLinkForDocumentLibrary() {
  const { id } = useParams();
  ...
  const handleSubmit = async () => {
    if (!saving) {
      ...
      if (validateEmail(email)) {
        try {
          setSaving(true);
          const emailLinkDTO: EmailLinkDTO = {
            registrationEventId: id!,
            email: email,
          };
          await agent.EmailLinks.sendLinkForDocumentLibrary(emailLinkDTO);
          toast.success(
            'a link to the document library was sent to your email',
          );
          setEmailSent(true);
        } catch (error: any) {
          console.log(error);
      ...
      <Container fluid>
        {!isMobile && (
          <Header
            as="h1"
            textAlign="center"
            style={{ textTransform: 'uppercase', fontSize: '3.5em' }}
            color="yellow"
          >
        ...
        {isMobile && (
          <Header
            as="h1"
            textAlign="center"
            style={{ textTransform: 'uppercase', fontSize: '2em' }}
            color="yellow"
          >
            Registration Portal
          </Header>
        ...
        {emailSent && (
          <Segment color="teal">
            <Header as="h2" icon textAlign="center">
              <Icon name="paper plane outline" />A Link to the Document Library
              Was Sent
              <HeaderSubheader>
                A link to access the document library has been sent to your
                email. Please check your inbox (and the spam folder, just in
                case) to access the document library.
              </HeaderSubheader>
            </Header>
