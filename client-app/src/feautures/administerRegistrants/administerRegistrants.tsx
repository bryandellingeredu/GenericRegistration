...
export default observer(function AdministerRegistrants() {
  ...
  const [showQuestions, setShowQuestions] = useState(() => {
    const storedShowQuestions = localStorage.getItem(
      LOCAL_STORAGE_KEY_SHOW_QUESTIONS,
    );
    return storedShowQuestions !== null
      ? JSON.parse(storedShowQuestions)
      : false;
  });
  useEffect(() => {
  ...
  const [queryOrder, setQueryOrder] = useState(() => {
    const storedQueryOrder = localStorage.getItem(
      LOCAL_STORAGE_KEY_QUERY_ORDER,
    );
    return storedQueryOrder !== null
      ? JSON.parse(storedQueryOrder)
      : 'registerDtAsc';
  });
  ...
  useEffect(() => {
    localStorage.setItem(
      LOCAL_STORAGE_KEY_SHOW_TABLE,
      JSON.stringify(showTable),
    );
  }, [showTable]);

  const handleSetShowQuestions = (newShowQuestions: boolean) =>
    setShowQuestions(newShowQuestions);
  const handleSetShowTable = (newShowTable: boolean) =>
    setShowTable(newShowTable);
  const handleSetQueryOrder = (newQueryOrder: string) =>
    setQueryOrder(newQueryOrder);
  const handleSetSearchFilter = (newSearchFilter: string) =>
  ...
  const handleDeleteRegistration = async (id: string) => {
    try {
      if (registrationEvent.registrations) {
        const registrations = registrationEvent.registrations.filter(
          (x) => x.id !== id,
        );
  ...
  const handleChangeRegistered = async (id: string, registered: boolean) => {
    try {
      if (registrationEvent.registrations) {
        const updatedRegistrations = registrationEvent.registrations.map(
          (r) => {
            if (r.id === id) {
  ...
  const getRegistrationEvent = async () => {
    ...
    try {
      ...
      const answerAttachmentData: AnswerAttachment[] =
        await agent.AnswerAttachments.listByEventRegistration(id!);
    ...
    <>
      ...
      <Container fluid style={{ color: '#333', paddingTop: '20px' }}>
          ...
          <div style={{ textAlign: 'center', width: '100%' }}>
            ...
            >
              ...
              <Header.Subheader style={{ color: '#666' }}>
                <h3>
                  Manage users that have registered for{' '}
        ...
        {(!registrationEvent ||
          ...
          registrationEvent.registrations.length < 1) && (
          <Message error content="No users have registered for this event" />
        )}
        {registrationEvent &&
          registrationEvent.registrations &&
          registrationEvent.registrations.length > 0 && (
