...
export default function AdministerRegistrantTableRow({
    ...
    .filter(
      (x) =>
        ...
        registration.answers?.find(
          (y) => y.answerText === x.parentQuestionOption,
        ),
    )
  ...
  const getAnswer = (questionId: string) => {
    const answerText =
      registration.answers!.find((x) => x.customQuestionId === questionId)
        ?.answerText || '';
    if (isGuid(answerText)) {
      const allOptions: QuestionOption[] = questions.reduce<QuestionOption[]>(
        (acc, question) => {
  ...
  const getAttachment = (questionId: string) => {
    ...
    const filteredAnswerAttachments = answerAttachments.filter(
      (x) => x.registrationLookup === registration.id,
    );
    if (filteredAnswerAttachments && filteredAnswerAttachments.length > 0) {
      return filteredAnswerAttachments.find(
  ...
  return (
    <>
      <TableRow style={style}>
        ...
        <TableCell
          rowSpan={maxRowSpan}
        >{`Registered on ${formattedDate}`}</TableCell>

        {showQuestions && orderedQuestions && orderedQuestions.length > 1 && (
          <>
            <TableCell>{orderedQuestions[0].questionText}</TableCell>
            <TableCell>
              {orderedQuestions[0].questionType !== QuestionType.Attachment &&
                getAnswer(orderedQuestions[0].id)}
              {orderedQuestions[0].questionType === QuestionType.Attachment &&
                !getAttachment(orderedQuestions[0].id) && <span></span>}
              {orderedQuestions[0].questionType === QuestionType.Attachment &&
                getAttachment(orderedQuestions[0].id) && (
                  <Button
                    animated="vertical"
                    size="tiny"
                    basic
                  ...
                  >
                    ...
                    <ButtonContent visible>
                      <Icon name="paperclip" />
                      {getAttachment(orderedQuestions[0].id)!.fileName ||
                        'file'}
                    </ButtonContent>
                  </Button>
                )}
        ...
        orderedQuestions.slice(1).map((question, index) => (
          <TableRow key={index}>
            ...
            <TableCell>
              ...
              {question.questionType === QuestionType.Attachment &&
                !getAttachment(question.id) && <span></span>}
              {question.questionType === QuestionType.Attachment &&
                getAttachment(question.id) && (
                  <Button
                    animated="vertical"
                    size="tiny"
                    basic
                  ...
                  >
                    ...
                    <ButtonContent visible>
                      <Icon name="paperclip" />
                      {getAttachment(question.id)!.fileName || 'file'}
                    </ButtonContent>
                  </Button>
                )}
            </TableCell>
