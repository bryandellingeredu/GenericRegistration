...
export default observer(function TreeNode({
  node,
  registrationEventId,
  isAdmin,
  encryptedKey,
}: Props) {
  const { documentLibraryStore, modalStore, attachmentStore, commonStore } =
    useStore();
  const { token } = commonStore;
  const { openModal, closeModal } = modalStore;
  const { edit, addFolder, deleteNode, addFile, moveNode } =
    documentLibraryStore;
  const { uploadDocumentLibraryDocument, setUploadingOn, setUploadingOff } =
    attachmentStore;
  ...
  async function handleUpload(files: File[], registrationEventId: string) {
    ...
    await Promise.all(
      files.map((file) => {
        ...
        return uploadDocumentLibraryDocument(
          file,
          answerAttachmentId,
          registrationEventId,
        ).catch((error) => {
    ...
    try {
      if (encryptedKey) {
        ...
        const answerAttachment: AnswerAttachment =
          await agent.EmailLinks.getAnswerAttachment(decodedKey, node.key);
        ...
        const answerAttachment: AnswerAttachment =
          await agent.AnswerAttachments.details(node.key);
    ...
    <>
      <Header as={children ? 'h2' : 'h3'}>
        {!showChildren && children && (
          <Icon name="folder outline" onClick={handleClick} />
        )}
        {showChildren && children && (
          <Icon name="folder open outline" onClick={handleClick} />
        )}
            ...
            <ButtonGroup size="tiny" style={{ marginLeft: '10px' }}>
              ...
              >
                <ButtonContent hidden>
                  {showChildren ? 'Close Folder' : 'Open Folder'}
                </ButtonContent>
