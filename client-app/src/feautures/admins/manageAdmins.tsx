...
import { Button, Container, Divider, Form, FormField, FormGroup, Header, Icon, Input, Label, Message, Segment, SegmentGroup } from 'semantic-ui-react';
...
export default observer(function ManageAdmin(){
    ...
    useEffect(() => {
        if (user && user.roles && user.roles.includes("Administrators")) setIsAdmin(true);
      }, [user]);
    ...
    useEffect(() => {
      ...
      const handleDeleteButton = async (email : string) => {
       ...
       const handleSubmit = async () => {
        ...
        if(adminEmails.map(x => x.toLowerCase()).includes(newAdminEmail.toLocaleLowerCase())){
            setNewAdminEmail('');
            return;
        } 
     ...
     const handleChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if(error){
            ...
            if(adminEmails.map(x => x.toLowerCase()).includes(emailInput.toLocaleLowerCase())){
                setNewAdminEmail('');
                return;
            } 
        ...
        {isAdmin && !loading && 
           ...
           <SegmentGroup style={{marginTop: '40px', padding: '20px'}}>
             <Segment textAlign="center" color="teal">
              ...
              <Segment clearing>
                <Button size='huge' primary floated='right' onClick={goBack} icon labelPosition='left'>
