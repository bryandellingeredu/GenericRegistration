...
export default class RegistrationStore {
  ...
  selectRegistration = (id: string) =>
    (this.selectedRegistration = this.registrationRegistry.get(id));

  cancelSelectRegistration = () => (this.selectedRegistration = undefined);

  setRegistrationLoadingInitial = (state: boolean) =>
    (this.registrationLoadingInitial = state);

  setRegistrationLoading = (state: boolean) =>
    (this.registrationLoading = state);

  addRegistration = (registration: Registration) =>
    this.registrationRegistry.set(registration.id, registration);
