...
export default class DocumentLibraryStore {
  ...
  saveToDB = async (registrationEventId: string, treeData: Node[]) => {
    if (store.userStore.user) {
      try {
        await agent.RegistrationEventDocumentLibraries.createUpdate(
          registrationEventId,
  ...
  fetchAndStoreTreeData = async (id: string) => {
    try {
      const response =
        await agent.RegistrationEventDocumentLibraries.details(id);
      if (response && response.treeData) {
        const parsedData = JSON.parse(response.treeData);
        runInAction(() => {
          this.addTreeData(
            id,
            parsedData && parsedData.length > 0 ? parsedData : defaultTreeData,
          );
        });
      } else {
        runInAction(() => {
  ...
  edit = (registrationEventId: string, key: string, newLabel: string) => {
    ...
    if (treeData) {
      // Check if treeData is not undefined
  ...
  moveNode = (
    ...
    const moveNodeInArray = (
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (
          nodes[i].children &&
          moveNodeInArray(nodes[i].children!, key, dir)
        ) {
          return true;
        }
      }
