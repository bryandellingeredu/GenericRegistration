...
{
    public class CreateForDocumentLibrary
    {
        ...
        public class Handler : IRequestHandler<Command, Result<Unit>>
        {
            ...
            public async Task<Result<Unit>> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                var registrationLinkUrl = $"{settings.BaseUrl}/documentlibraryfromlink?key={urlEncodedEncryptedKey}";

                GraphHelper.InitializeGraph(settings, (info, cancel) => Task.FromResult(0));
                string title = $"Review Documents For {registrationEvent.Title}";
                string body = $"Please review documents for {registrationEvent.Title} before the start of the event : <a href=\"{registrationLinkUrl}\">Document library for {registrationEvent.Title}</a>";
                try
                {
